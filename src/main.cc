...
namespace __startup
{
	void loadPlayerBase(qData *inData)
	{
	...
	void loadAll(qData *inData)
	{
	    loadPlayerBase(*&inData);
		loadMap0(*&inData);
	};

	void setRenderModels(qData *inData)
	{
		qData::qFile::qModel *localmap0 = inData->file.findGetModel("map0");
		qData::qFile::qModel *localplayer = inData->file.findGetModel("player");
...
{
	void keyWUp(qData *inData){inData->session.render.scene.findGetModel("player0")->currentAnim = 0;};
	void keySUp(qData *inData){inData->session.render.scene.findGetModel("player0")->currentAnim = 0;};
    void keyWDown(qData *inData)
    {
    	inData->session.render.scene.findGetModel("player0")->currentAnim = 1;
        inData->world.player[0].move();
        
    };
    void keySDown(qData *inData)
    {
    	inData->session.render.scene.findGetModel("player0")->currentAnim = 1;
        inData->world.player[0].move(true);
    };
    void keyADown(qData *inData)
    {
        inData->world.player[0].setRotationY(qMath::round360(inData->world.player[0].getRotation()->y));
        inData->world.player[0].rotate();
    };
    void keyDDown(qData *inData)
    {
        inData->world.player[0].setRotationY(qMath::round360(inData->world.player[0].getRotation()->y));
