	...
	world.creature.new = function(ref)
	{
		/*
	 	ref must follow this template:
	 	{
			...
			model:partido.file.model['model name'],
			texture:partido.file.image['text name']//TEXTURE IS OPTIONAL
		}
		*/
		...
		/* 	ref must follow this template:
	 	{
			...
			rotation:{x:0,y:0},
			scale:{w:100,h:100},
			image:partido.file.image['image name']
		}
		*/
		this[ref.name] = ref;
		this[ref.name].active = true;
...
class Partido
{
	...
	setup = function()
	{
		this.canvas = createCanvas(partido.screen.w, partido.screen.h, P2D);//MAIN RENDERER USED FOR UI
		this.gfx = createGraphics(partido.screen.w, partido.screen.h, WEBGL);//SECONDARY RENDERER USED FOR ALL 3D STUFF, THIS IS RENDERED IN MAIN RENDERER AS A IMAGE
		...
		this.gfx.perspective(PI / 3.0, partido.screen.w / partido.screen.h, 0.001, 1000);
...
function keyPressed()
{
	...
	{
		player.model = partido.file.model['player-walk'];
		player.currentProgression = -1;
		if(frameCount%10)
			partido.scene.camera.update();
	}
...
function keyReleased()
{
	...
	{
		player.model = partido.file.model['player-idle'];
	}
}
...
function preload()
{
	...
	partido.file.image.load('assets/models/map/level0/texture_0.png','map0');//single-file image import example
}
...
function setup()
{
	...
	({
		...
		scale:{x:1,y:1,z:1},
		model:partido.file.model['player-idle'],
		hitbox:{}
	});
	...
	map = 
	{
		...
		scale:{x:1,y:1,z:1},
		model:partido.file.model['map0'],
		texture:partido.file.image['map0']
	}
	partido.scene.model.add(map);
	partido.scene.model.add(player);
}

function draw() 
{
	partido.gfx.clear();
	partido.gfx.noStroke();
	//keyDown();
	
	partido.scene.render();
	image(partido.gfx, 0, 0, partido.screen.w, partido.screen.h);
