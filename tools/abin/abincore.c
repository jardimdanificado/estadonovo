...
void abinCoreAddNewData(char *fileloc, char *type, char *new)
{
    ...
    char* bufferzao;
    bufferzao = malloc(512);
    snprintf(bufferzao,512,"%s %s\n",type,new);
    fputs(bufferzao, file);
    fclose(file);
    free(bufferzao);
}

FILE* abinOperativeAdd(FILE *file, char *type, char *new)
{
    snprintf(ABINCACHE512,512,"%s %s\n",type,new);
    fputs(ABINCACHE512, file);
    return file;
}
